...
- **DONE evidence check** — accept a DONE only after verifying: (1) commit hash exists in `git log`, (2) message matches the repo convention, (3) changed files ⊆ task boundary (`git show --name-only <hash>`), (4) **schema check (GH-40):** pipe the DONE report through `story-report-validate` (stdin) — exit 1 (FAIL: MISSING-FIELD/BAD-STATUS/NONE-ON-DONE/UNCLOSED-REPORT) → coi DONE đó là BLOCKED, re-dispatch kèm đúng token để executor sửa trong retry budget. Report không fence → WARN `LEGACY-REPORT` exit 0 (chấp nhận phiên này, FAIL từ kit 2.8.0). Any mismatch → treat as BLOCKED, re-dispatch per ledger. A DONE without evidence is not a DONE.
