...
- Get push notifications when an agent finishes or a story decision gate opens, mirroring [desktop notifications](/docs/notifications) — tapping a gate notification jumps straight to its story.
- Follow a host's superpowers **stories** — progress, per-SF status by tier, pending decision gates — and resolve a gate from the phone.
...
## Stories and decision gates

Hosts running superpowers stories show them under **Stories** in the host screen header (next to **Gates**):

- The story list groups stories by worktree. Each row shows the epic, progress ("2/5 SF done"), and an amber badge with the number of pending gates; pull to refresh.
- Tapping a story opens the detail view: a progress bar, SFs grouped by tier with status chips (Todo / In progress / Done), and the story's decision gates.
- Pending gates resolve from the phone — in the story's gates section or from the **Gates** screen, which lists every pending gate on the host grouped by story (gates without a linked story sit under **Khác**, and story-linked rows link to the story detail). Pick one of the gate's options — or type a free-text resolution when the options aren't known — and confirm the dialog. If the gate was already handled on desktop, the phone says so instead of erroring.
