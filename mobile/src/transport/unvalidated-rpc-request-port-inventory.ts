...
export const UNVALIDATED_RPC_REQUEST_PORT_OWNERS: readonly UnvalidatedRpcRequestPortEntry[] = [
  ...
  { file: 'src/transport/rpc-client.ts', references: 1 },
...
export const UNVALIDATED_RPC_REQUEST_PORT_PENDING: readonly UnvalidatedRpcRequestPortEntry[] = [
  ...
  // src/superpowers/ — the fork's story and gate surfaces. The two story reads and the sweep
  // coalesce through sendSingleFlightRequest (one in-flight read per client+host), the sweep's
  // story-detail leg adds a per-story in-flight map on top of that, and the gate resolve must
  // never share an in-flight call — none of that maps onto one operation until the operation
  // layer grows an operation-aware coalescer. The unsubscribe rides in a cleanup path whose
  // result is intentionally discarded.
  { file: 'src/superpowers/gate-resolve-request.ts', references: 1 },
  { file: 'src/superpowers/gate-transition-events.ts', references: 1 },
  { file: 'src/superpowers/pending-gates-sweep.ts', references: 3 },
  { file: 'src/superpowers/story-detail-host-fetch.ts', references: 2 },
  { file: 'src/superpowers/story-list-host-fetch.ts', references: 2 },
  ...
  // src/tasks/ — Wakii holdouts. The fork's provider/detail/project action modules predate the
  // RpcOperation migration and were kept at the fork's recorded state (see the tasks parity
  // family); each sends raw through the shared client until the fork adopts the operation layer.
