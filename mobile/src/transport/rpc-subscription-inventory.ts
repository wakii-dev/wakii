...
export const RPC_SUBSCRIPTION_SITES: readonly RpcSubscriptionSite[] = [
  ...
  // principle; no scenario drives a gate event yet.
  {
    file: 'src/superpowers/gate-transition-events.ts',
    method: 'notifications.subscribe',
    coverage: { kind: 'unwritten-scenario' }
  },
  // Wakii superpowers: polling refresh is local, the stream only nudges it. Same mountability.
  {
    file: 'src/superpowers/story-screen-refresh.ts',
    method: 'runtime.clientEvents.subscribe',
    coverage: { kind: 'unwritten-scenario' }
  }
]
