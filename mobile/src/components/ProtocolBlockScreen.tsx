...
function blockTitle(remedy: BlockRemedy): string {
  switch (remedy) {
    case 'update-mobile':
      return 'Update Wakii Mobile'
    case 'update-desktop':
      return 'Update Wakii on your computer'
    case 'refresh-bundle':
...
function blockBody(verdict: BlockedVerdict, remedy: BlockRemedy, storeName: string): string {
  ...
  if (verdict.reason === 'mobile-too-old') {
    return `This desktop needs a newer Wakii Mobile app. Update Wakii Mobile from ${storeName}, then try this host again.`
  }
  if (verdict.reason === 'bundle-unavailable') {
    return 'This paired desktop app does not include the mobile workspace yet. Update Wakii on your computer, then try this host again.'
  }
  if (remedy === 'update-mobile') {
    return `This desktop's mobile workspace needs a newer Wakii Mobile app. Update Wakii Mobile from ${storeName}, then try this host again.`
  }
