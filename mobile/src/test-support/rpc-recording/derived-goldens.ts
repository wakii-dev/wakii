...
// Wakii: the fork's aivault/push/pr consumers diverge from upstream's wire contract on these
// partitions (they predate the fork's RpcOperation migration), so their matrix goldens cannot
// record here. The fork's earlier sync pruned their scenarios for the same reason.
const FORK_EXCLUDED_MATRIX_IDS = new Set([
  ...
  'matrix-project-explicit-false-github.project.updateissuebyslug-1'
])
...
export function familyGoldens(manifest: readonly RecordingScenario[]): DerivedGolden[] {
  ...
  for (const [family, scenarios] of families) {
    ...
    for (const site of replyMatrixSites(base)) {
      ...
      if (FORK_EXCLUDED_MATRIX_IDS.has(id)) {
        continue
      }
      if (ids.has(id)) {
