...
1. **Sentinels + keep-deleted policy**: fork-only files still exist (launcher kit, mobile superpowers screens). The fork keeps **only the release workflow chain** — `fork-release-cut`, `macos-build`, `windows-build`, `mobile-android-release`, `mobile-ios-release`, `release-mac-build`, `sync-fork` — every other upstream workflow stays deleted; merges keep resurrecting them (`pr.yml`, `mobile.yml`, `unit-tests`, `e2e`, `cloud-*`, ...). Delete again on sight.
...
8. **Built-in skill manifest**: after touching `skills/` (the 6 Wakii stateless skills live there since 2.16.0 alongside upstream's 8), run `node config/scripts/generate-skill-bundle-manifest.mjs` (verify mode, no `--write`) — must pass. The full `verify-skill-update-roundtrip.mjs` e2e requires the `skills` npm package that only upstream publishes; locally the generator-verify is the equivalent gate.

CI no longer runs anywhere except the release chain above (owner request): there are no PR checks, no unit-test or e2e workflows. This battery is the only gate before pushing a sync or feature merge to `wakii-dev`, and packaging gates in the release build are the only automated nets after it.
