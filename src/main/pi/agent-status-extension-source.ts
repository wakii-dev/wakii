...
export function getPiAgentStatusExtensionSource(kind: PiAgentKind = 'pi'): string {
  ...
  return [
    ...
    '// Wakii receiver from building an unbounded queue of obsolete snapshots.',
    ...
    '    timeout = setTimeout(() => {',
    '      controller?.abort()',
    "      reject(new Error('Wakii hook delivery timed out'))",
    ...
    '    ])',
    '  } catch {',
    '    // Why: status reporting must never fail the pi run just because Wakii',
    '    // is unavailable or the loopback request failed (e.g. Wakii restart).',
