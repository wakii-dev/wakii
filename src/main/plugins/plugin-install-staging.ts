...
async function readInstallManifest(
  ...
  if (!satisfiesOrcaEngineRange(hostVersion, parsed.manifest.engines.orca)) {
    return {
      ok: false,
      error: `plugin requires Wakii ${parsed.manifest.engines.orca} (this is ${hostVersion})`
    }
  }
...
export async function installStagedPluginTree(input: {
  ...
  if (blockedReason) {
    return { ok: false, error: `plugin is blocked by Wakii's safety list: ${blockedReason}` }
  }
