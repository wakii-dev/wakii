export function getStatusPluginEndpointSource(): string[] {
  return [
    ...
    "// OpenCode's Node process (not Wakii's) and has no access to server.ts's",
    ...
    '// which reads the endpoint file. The file only changes on Wakii restart',
    ...
    '      // writeEndpointFile on the Wakii side) allocates a fresh inode on',
    ...
    '  // frozen when OpenCode was fork()ed — stale after an Wakii restart. The',
    '  // file is rewritten on every Wakii start(), so sourcing it per post lets',
    '  // a long-running OpenCode session reach the current server. Falls back',
    '  // to process.env when the file is absent (first-run / pre-endpoint-file / Wakii',
