...
function makeHostStatus(
  launcherPath = 'C:\\Users\\me\\AppData\\Local\\Programs\\Wakii\\resources\\bin\\orca.exe'
) {
  return {
    platform: 'win32',
    commandName: 'orca',
    commandPath: launcherPath,
    pathDirectory: 'C:\\Users\\me\\AppData\\Local\\Programs\\Wakii\\resources\\bin',
    pathConfigured: true,
    launcherPath,
...
if command -v powershell.exe >/dev/null 2>&1; then
  ...
  echo "Wakii WSL CLI requires Windows interop and could not find powershell.exe." >&2
  exit 1
fi
  ...
  const runner = vi.fn(async (_distro: string, command: string) => {
    ...
    if (command.includes('cat > "$command_tmp"')) {
      ...
      if (
        ...
        !files.get(bridgePath)?.includes('# Wakii managed WSL CLI PowerShell bridge')
      ) {
      ...
      if (files.get(legacyCommandPath)?.includes('# Wakii managed WSL CLI launcher')) {
    ...
    if (command.includes('rm -f')) {
      ...
      if (
        ...
        files.get(legacyCommandPath)?.includes('# Wakii managed WSL CLI launcher')
      ) {
        files.delete(legacyCommandPath)
      }
      return ''
...
describe('WslCliInstaller', () => {
  ...
  it('installs a WSL launcher that forwards to the Windows Wakii launcher', async () => {
    ...
    expect(wsl.getFile()).toBe(
      _internals.buildWslLauncher(
        'C:\\Users\\me\\AppData\\Local\\Programs\\Wakii\\resources\\bin\\orca.exe',
  ...
  it('reports installed WSL launchers whose bin directory is missing from PATH', async () => {
    ...
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
      hostInstaller: { getStatus: async () => makeHostStatus('C:\\Wakii\\orca.cmd') },
      wslRunner: wsl.runner
    })
  ...
  it('accepts current managed WSL scripts with an extra heredoc trailing newline', async () => {
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'installed',
      currentTarget: 'C:\\Wakii\\orca.cmd'
    })
  })
  ...
  it('generates a launcher that forwards arguments through a PowerShell file bridge', () => {
    const launcher = _internals.buildWslLauncher(
      'C:\\Program Files\\Wakii\\resources\\bin\\orca.exe',
      '/home/alice/.local/share/orca/orca-wsl-bridge.ps1'
    )
    ...
    expect(launcher).toContain(
      'Wakii WSL CLI requires Windows interop and could not find powershell.exe.'
    )
    ...
    expect(bridge).toContain('[string]$WakiiLauncher = $args[0]')
    ...
    expect(bridge).toContain('$LauncherDirectory = Split-Path -Parent $WakiiLauncher')
    ...
    expect(bridge).not.toContain('& $WakiiLauncher @ForwardArgs')
  ...
  it('marks stale managed launchers that point at the old app bin instead of packaged resources', async () => {
    const oldLauncher = _internals.buildWslLauncher(
      'C:\\Users\\me\\AppData\\Local\\Programs\\Wakii\\bin\\orca.cmd',
      '/home/alice/.local/share/orca/orca-wsl-bridge.ps1'
    )
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'stale',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\Wakii\\bin\\orca.cmd',
      launcherPath: 'C:\\Users\\me\\AppData\\Local\\Programs\\Wakii\\resources\\bin\\orca.exe'
    })

    await expect(installer.install()).resolves.toMatchObject({
      state: 'installed',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\Wakii\\resources\\bin\\orca.exe'
    })
  })

  it('repairs the frozen pre-rc4 registration so orchestration send/reply reach native rc4', async () => {
    const nativeLauncher = 'C:\\Program Files\\Wakii\\resources\\bin\\orca.exe'
  ...
  it('repairs a managed launcher whose bridge is missing, but preserves a conflicting bridge', async () => {
    const nativeLauncher = 'C:\\Wakii\\resources\\bin\\orca.exe'
    ...
    const staleBridge = createWslRunner(PRE_RC4_MANAGED_WSL_LAUNCHER, true, {
      initialBridge: '# Wakii managed WSL CLI PowerShell bridge\nWrite-Output "stale"\n'
    })
  ...
  it('keeps the pre-rc4 files on a transactional replacement failure', async () => {
    ...
    const installer = new WslCliInstaller({
      ...
      distro: 'Ubuntu',
      hostInstaller: {
        getStatus: async () => makeHostStatus('C:\\Program Files\\Wakii\\resources\\bin\\orca.exe')
      },
      wslRunner: wsl.runner
    })
  ...
  it('is idempotent after repairing an old managed registration', async () => {
    const nativeLauncher = 'D:\\Custom Wakii\\resources\\bin\\orca.exe'
    const wsl = createWslRunner(PRE_RC4_MANAGED_WSL_LAUNCHER)
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
