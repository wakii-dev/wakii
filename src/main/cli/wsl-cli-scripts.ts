const MANAGED_MARKER = '# Wakii managed WSL CLI launcher'
const BRIDGE_MANAGED_MARKER = '# Wakii managed WSL CLI PowerShell bridge'
...
try {
  ...
  if ($args.Count -lt 1) {
    throw 'Invalid Wakii WSL CLI bridge invocation.'
  }
  ...
  if ($null -eq $Process) {
    throw 'Unable to start the Wakii Windows CLI launcher.'
  }
  $Process.WaitForExit()
