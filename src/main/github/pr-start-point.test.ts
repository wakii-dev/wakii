...
describe('resolveGitHubPrStartPoint', () => {
  ...
  it('prefers the pull-head error when the branch miss triggered a failing fallback', async () => {
    ...
    fetchPullRequestHeadRefMock.mockRejectedValue(
      new Error(
        'This SSH host is running an older Wakii relay that cannot fetch pull request heads.'
      )
    )
    ...
    expect(result).toEqual({
      error:
        'Failed to fetch refs/pull/77/head: This SSH host is running an older Wakii relay that cannot fetch pull request heads.'
    })
  })
