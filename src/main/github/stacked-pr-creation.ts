...
export async function prepareGitHubStackedPullRequest(
  ...
  try {
    ...
    if (parentPullRequests.length !== 1) {
      return creationError(`Wakii found multiple open pull requests for the parent branch ${base}.`)
    }
    if (currentPullRequests.length > 1) {
      return creationError(`Wakii found multiple open pull requests for the current branch ${head}.`)
    }
    ...
    return {
      ...
      error: isStacksUnavailableError(error)
        ...
        : 'Wakii could not verify the parent pull request. Retry in a moment.'
    }
  } finally {
    release()
  }
