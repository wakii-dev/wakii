...
function buildInstallFixture(root: string): void {
  ...
  writeFileSync(join(root, 'Wakii.exe'), 'exe-bytes')
...
beforeEach(() => {
  ...
  setProcessProp('execPath', join(installDir, 'Wakii.exe'))
...
describe('buildDaemonHostManifest', () => {
  it('mirrors the win-unpacked layout: exe + data blobs + resources tree, no GPU DLLs', () => {
    ...
    expect(byDest.get('Wakii.exe')?.kind).toBe('file')
    const exeOp = ops.find((op) => op.sourcePath === 'C:\\app\\Wakii.exe')
    expect(exeOp?.destRel).toBe('Wakii.exe')
...
describe('materializeRelocatedDaemonHost', () => {
  it('copies the tree, writes the marker, and returns mirrored fork paths', () => {
    ...
    const dest = join(localAppDataDir, 'Wakii', 'daemon-host', '9.9.9')
    expect(result?.execPath).toBe(join(dest, 'Wakii.exe'))
  ...
  it('copies the exe verbatim: same file name and same bytes as the install-dir exe', () => {
    const result = materializeRelocatedDaemonHost()
    const sourceExe = join(installDir, 'Wakii.exe')
  ...
  it('tracks a differently-named app exe rather than pinning an image name of its own', () => {
    ...
    renameSync(join(installDir, 'Wakii.exe'), join(installDir, 'Wakii Nightly.exe'))
    setProcessProp('execPath', join(installDir, 'Wakii Nightly.exe'))
    const result = materializeRelocatedDaemonHost()
    const dest = join(localAppDataDir, 'Wakii', 'daemon-host', '9.9.9')
    expect(result?.execPath).toBe(join(dest, 'Wakii Nightly.exe'))
    ...
    expect(getRelocatedDaemonHost()?.execPath).toBe(join(dest, 'Wakii Nightly.exe'))
  })
  ...
  it('fails open on a missing required input, leaving no dest or staging dir', () => {
    ...
    const hostRoot = join(localAppDataDir, 'Wakii', 'daemon-host')
  ...
  it('returns null off win32', () => {
    ...
    expect(existsSync(join(localAppDataDir, 'Wakii', 'daemon-host'))).toBe(false)
  })
...
describe('getRelocatedDaemonHost', () => {
  it('returns null when the marker version does not match the current version', () => {
    ...
    writeFileSync(join(dest, 'Wakii.exe'), 'exe')
...
describe('pruneOldDaemonHosts', () => {
  it('removes unpinned non-current version dirs, keeping current and pinned', () => {
    const root = join(localAppDataDir, 'Wakii', 'daemon-host')
