...
describe('registerPtyHandlers', () => {
  ...
  describe('spawn environment', () => {
    ...
    it('restores user Pi config when agent status hooks are disabled in a nested Wakii shell', async () => {
      const env = await spawnAndGetEnv(
        {
          PI_CODING_AGENT_DIR: '/tmp/parent-orca-pi-overlay',
          ORCA_PI_CODING_AGENT_DIR: '/tmp/parent-orca-pi-overlay',
    ...
    it('injects the agent hook receiver env into Wakii terminal PTYs', async () => {
      const env = await spawnAndGetEnv()
    ...
    it('overrides ambient CODEX_HOME with the Wakii-managed home for system default', async () => {
      const env = await spawnAndGetEnv(
        undefined,
    ...
    it('strips a nested-Wakii override for system default when the real-home flag is ON', async () => {
      const env = await spawnAndGetEnv(
