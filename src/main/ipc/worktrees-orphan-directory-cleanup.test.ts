...
describe('registerWorktreeHandlers', () => {
  ...
  it('force-removes a legacy Wakii-created orphaned worktree directory after Git tracking is gone', async () => {
  ...
  it('prompts for force before removing an Wakii-created orphaned worktree directory', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'orca-ipc-orphan-'))
    const repoPath = join(parentDir, 'repo')
    const orphanPath = join(parentDir, 'orphan')
    const adminWorktreePath = join(repoPath, '.git', 'worktrees', 'orphan')
  ...
  it('prompts then force-removes an Wakii-created unregistered leftover directory with no git marker', async () => {
  ...
  it('rejects an Wakii-created unregistered local directory with a git directory', async () => {
