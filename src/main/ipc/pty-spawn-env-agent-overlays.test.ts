...
describe('registerPtyHandlers', () => {
  ...
  describe('spawn environment', () => {
    ...
    it('injects the OpenCode hook env into Wakii terminal PTYs', async () => {
    ...
    it('restores user OpenCode config when agent status hooks are disabled in a nested Wakii shell', async () => {
      const env = await spawnAndGetEnv(
        {
          OPENCODE_CONFIG_DIR: '/tmp/parent-orca-opencode-overlay',
          ORCA_OPENCODE_CONFIG_DIR: '/tmp/parent-orca-opencode-overlay',
    ...
    it('restores user MiMo home when agent status hooks are disabled in a nested Wakii shell', async () => {
      const env = await spawnAndGetEnv(
        {
    ...
    posixOnlyIt(
      'reproduces issue #1534: GUI-launched Wakii mirrors zshrc-only OpenCode config',
      async () => {
    ...
    it('installs Pi managed extensions without redirecting Wakii terminal PTY homes', async () => {
    ...
    it('mirrors the original Pi source dir when launched from an Wakii overlay shell', async () => {
      const env = await spawnAndGetEnv({
