...
export function buildAgentSessionContinuationPrompt(
  ...
  const sourceLines = [
    ...
    source.sourceLabel ? `Wakii pane: ${source.sourceLabel}` : null,
  ...
  return [
    'Continue work from the prior Wakii session using the context below.',
    ...
    ...(statusHints.length > 0 ? ['', 'Latest Wakii status hints:', ...statusHints] : []),
    '',
