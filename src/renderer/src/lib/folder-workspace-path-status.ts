...
export function getFolderWorkspacePathStatusDescription(
  ...
  if (!isHandledPathStatusReason(status.reason)) {
    return translate(
      ...
      'Wakii cannot use {{path}}, and this version does not recognize the reason. Update Wakii to see the details.',
      { path: status.path }
    )
  }
  switch (status.reason) {
    case 'missing':
      return translate(
        ...
        'Wakii cannot find {{path}}. Remove and re-import this folder workspace.',
        { path: status.path }
      )
    case 'not-directory':
      return translate(
