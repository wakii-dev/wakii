...
it('lists one row per paired Wakii server under this computer, and says where SSH stands', async () => {
  mocks.environments = [
  ...
  expect(screen.getByText('Wakii remote servers')).toBeInTheDocument()
})
...
it('leaves a lone computer to its own switch, with no roll-up above it', async () => {
  ...
  expect(screen.queryByText('Wakii remote servers')).not.toBeInTheDocument()
})
