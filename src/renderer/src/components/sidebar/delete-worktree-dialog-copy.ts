  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from Wakii. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from Wakii. Git worktrees will also be removed from git and disk; folder workspaces will only remove the Wakii workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from Wakii. The project folder on disk will not be deleted.'
