...
function humanizePermission(permission: string): string {
  switch (permission) {
    ...
    case 'openExternal':
      return 'permission to open a link outside Wakii'
    case 'fileSystem':
...
export function formatPermissionNotice(event: BrowserPermissionDeniedEvent): string {
  ...
  return `${target} asked for ${humanizePermission(event.permission)}, and Wakii denied it.`
}
...
export function formatPopupNotice(event: BrowserPopupEvent): string | null {
  ...
  return `${target} tried to open a popup Wakii does not support here.`
}
  ...
  if (isChromiumCertificateErrorCode(loadError.code)) {
    ...
    if (loadError.code === -202) {
      return translate(
        ...
        "Wakii doesn't trust the authority that issued the certificate for {{value0}}.",
        { value0: host }
      )
    }
    return translate(
      'browser.loadFailure.certificateVerificationFailed',
      "Wakii couldn't verify the certificate for {{value0}}.",
      { value0: host }
    )
  }
