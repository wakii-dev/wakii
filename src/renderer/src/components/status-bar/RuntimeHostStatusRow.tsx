...
function runtimeFailureExplanation(state: RuntimeHostConnectionState): string | null {
  ...
  if (state === 'runtime-unavailable') {
    return translate(
      ...
      'The remote host may still be running; only the Wakii runtime connection is unavailable.'
    )
  }
  return translate(
    'auto.components.status.bar.RuntimeHostStatusRow.contact_note',
    'The host may still be running; only the Wakii connection is unavailable.'
  )
}
