...
async function assertProjectHostSetupRuntimeCapability(target: RuntimeClientTarget): Promise<void> {
  ...
  await assertRuntimeEnvironmentCapability(
    ...
    PROJECT_HOST_SETUP_RUNTIME_CAPABILITY,
    'The selected Wakii server does not support project host setup yet. Update Wakii on the server and try again.',
    15_000
  )
}
...
export async function assertProjectHostSetupMutationRuntimeCapabilities(
  ...
  await assertRuntimeEnvironmentCapability(
    target.environmentId,
    WORKSPACE_RUN_CONTEXT_RUNTIME_CAPABILITY,
    'The selected Wakii server does not support explicit workspace run hosts yet. Update Wakii on the server and try again.',
    15_000
  )
}
